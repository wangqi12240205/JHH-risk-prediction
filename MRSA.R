...
get_censored <- function(Xs, Y_k) {
  ...
  CT = data.frame(T_start, T_end, censored, Xs)
...
for (k in 1:length(unit_name)) {
  ...
  X_k = X_Tbl[index,];
  dp[[k]] = get_censored(X_k, Y_k);
  dp_all = rbind(dp_all, dp[[k]]);
  cat(sprintf("\n***** UNIT %s *****\n",unit_name[k]))
  res.cox_each[[k]] <- coxph(Surv(T_start, T_end, censored) ~ X1 + X2 + X3, data =dp[[k]])
  betas = rbind(betas, res.cox_each[[k]]$coefficients) 
}
res.cox_all <- coxph(Surv(T_start, T_end, censored) ~ X1 + X2 + X3, data = dp_all)
par(mfrow=c(1,1))
